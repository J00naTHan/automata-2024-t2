    ...
    try:
        with open(filename, "rt") as file:
            ...
            for rule in lines[4:]:
                ...
                if len(rule) == 3:
                    if rule[0] in automata['Q'] and rule[2] in automata[
                            'Q'] and rule[1] in automata['sigma']:
                        if rule[0] not in delta:
                            delta[rule[0]] = {}
                        if rule[1] not in delta[rule[0]]:
                            delta[rule[0]][rule[1]] = [rule[2]]
                        else:
                            delta[rule[0]][rule[1]].append(rule[2])
                            if automata['is_nfa'] is False:
                                automata['is_nfa'] = True
                    else:
                        raise Exception(
                            f'transition rule states or symbol not valid')
                else:
                    raise Exception(
                        'transition rule has more or less than 3 parameters')
...
def epsilon_closures(Q, q0, delta, epsilon_symbol='&'):
    ...
    for q in Q:
        ...
        for e in e_closure:
            try:
                epsilon_rules = delta[e][epsilon_symbol]
...
def convert_to_dfa(automata):
    ...
    if isinstance(automata, dict):
        ...
        raise Exception(
            f'automata expected type: <dict>, received type: <{type(automata)}>'
        )
            ...
            for sub_q in q:
                ...
                try:
                    for rule in delta[sub_q][symbol]:
                        if rule not in q_rules:
                            q_rules.append(rule)
                except KeyError:
                    None
            nq_set = set(q_rules).union(q_closure)
            if nq_set not in nQ_set:
                nQ_set.append(nq_set)
            nq_set = list(nq_set)
            for q in nq_set:
                if q == nq_set[-1]:
                    nq += q
